...
- (void)viewDidLoad
{
   ...
   //un dito swipe verso sinistra
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        {
            NSLog(@"Y SCROLL VIEW = %f, Y VIEW = %f, Y BAR = %f",scrollView.frame.origin.y,self.view.frame.origin.y,self.navigationController.navigationBar.frame.origin.y+self.navigationController.navigationBar.frame.size.height);
            
            // iPhone 5
            [scrollView setFrame:CGRectMake( 0, -50, scrollView.frame.size.width, scrollView.frame.size.height)];
            [pageControl setFrame:CGRectMake(pageControl.frame.origin.x, pageControl.frame.origin.y+30, pageControl.frame.size.width, pageControl.frame.size.height)];
        }
    }
}
